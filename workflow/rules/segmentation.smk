...
rule tissue_seg_kmeans_init:
    ...
    output:
        seg = bids(root='results',subject='{subject}',suffix='dseg.nii.gz',desc='atroposKseg'),
        posteriors = bids(root='results',subject='{subject}',suffix='probseg.nii.gz',desc='atroposKseg'),
    shadow: 'minimal'    
...
rule tissue_seg_to_4d:
    input:
        tissue_segs = expand(bids(root='results',subject='{subject}',suffix='probseg.nii.gz',label='{tissue}',desc='atropos3seg'),
                            tissue=config['tissue_labels'],allow_missing=True),
    output:
        tissue_seg = bids(root='results',subject='{subject}',suffix='probseg.nii.gz',desc='atropos3seg')
    group: 'preproc'
    container: config['singularity']['neuroglia']
    shell:
